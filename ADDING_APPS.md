...
## 7. Додавання апок з Windows на Linux-сервер (щоб грузились в клієнт у runtime)

Якщо **сервер платформи на Linux**, а ти **розробляєш і збираєш на Windows**, щоб нова апка з’явилась на сервері і клієнт (.exe) її завантажував у runtime:

**Крок 1. Апка на Windows**

- Створи/змінь код у `runtime_logic/apps/<app_id>/` (як у розділах 1–3 вище).

**Крок 2. Збірка runtime-пакета на Windows**

```bash
python manager.py build -a <app_id> -v 1.0.0 -c stable
```

У проєкті з’являться файли в `backend/packages/` (manifest `.json` + архів `.zip`). Саме вони потрібні серверу.

**Крок 3. Вигрузка пакетів на Linux-сервер**

Використовуй команду **deploy** (вигрузка по SFTP у `backend/packages/` на сервері):

```bash
python manager.py deploy --build
```

Або якщо вже збирав раніше — тільки вигрузка (без `--build`), за умови що вказано хост/юзер/шлях (через `deploy_config.json` або аргументи):

```bash
python manager.py deploy
```

Після цього на сервері в каталозі проекту будуть оновлені файли в `backend/packages/` (усі `.json` і `.zip`, включно з новою апкою).

**Крок 4. Оновлення списку релізів на сервері**

Бекенд підхоплює пакети з папки при **старті** або після **ручної синхронізації**. Після вигрузки з Windows варто одне з двох:

- **Перезапустити backend** на Linux (тоді при старті викликається `sync_from_packages_dir`), або  
- У **адмін-панелі** на сервері (https://твій-домен/admin/) відкрити вкладку **Releases** і натиснути **«Sync Packages»**. Список релізів оновиться, нова апка з’явиться в таблиці.

**Крок 5. Ліцензії для нової апки**

У адмінці створи нову ліцензію з `channel_access` для цієї апки, наприклад `{"<app_id>": ["stable"]}`, або для існуючої ліцензії додай у `channel_access` ключ `<app_id>` з потрібними каналами (див. розділ 5).

**Крок 6. Клієнт і runtime**

- Якщо клієнт (.exe) вже зібраний з `--server-url` твого Linux-сервера — користувач просто вводить ключ; клієнт підключається до сервера, отримує маніфест для дозволеної апки і **завантажує runtime** (той самий .zip з кроку 2, тепер вже з сервера).
- Якщо збираєш окремий exe під цю апку (з іконкою): на Windows `python manager.py client -a <app_id> -n MyAppBootstrap --server-url https://твій-сервер`. Потім можна тим самим deploy вигрузити лише пакети (крок 3), exe залишається в `dist/` для роздачи.

**Коротко:** апки «додаються на сервер» тим, що ти вигружаєш зібрані `backend/packages/` на Linux (deploy). Після Sync у адмінці (або перезапуску backend) релізи оновлюються, і клієнт при введенні ключа завантажує потрібний runtime з сервера.
